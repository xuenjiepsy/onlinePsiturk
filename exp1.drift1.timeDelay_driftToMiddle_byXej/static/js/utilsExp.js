...
function judgePressKeyAndClick(info, agentIdSeleted, interval, trajFrame, clickListX, clickListY, pressNum, trialIndex, ifText){
  ...
  document.addEventListener('click', (e) => {
    ...
    if (clickListX.includes(mouseX)&&clickListY.includes(mouseY)){}
      ...
      agentIdSeleted = checkSelection(mouseX, mouseY, trajData[trialIndex][trajFrame], agentIdSeleted)
    }
    
    
  })
  // jsPsych.data.get().addToLast({trajFramePause: trajFrame}) 
  jsPsych.data.get().addToLast({agentIdSeleted: agentIdSeleted}) 
  clearInterval(interval);
  return 'j';}
}

function checkSelection(mouseX, mouseY, traj, agentIdSeleted) {
